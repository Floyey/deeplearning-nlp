...
st.sidebar.link_button("📄 Lien vers le dataset utilisé", "https://www.kaggle.com/datasets/basilb2s/language-detection",)

# Input
sentence_identification = st.text_input("Your sentence")

if sentence_identification != "":
    language_prediction = model_prediction(sentence_identification)
    st.write("Your sentence is in: ", "**"+language_prediction+"**")
...
if st.checkbox("Do you want to know the translated sentence?"):
    ...
    if sentence_identification != "" and language_selected != "":
        ...
        if st.button("Translate"):
            translated_sentence = translate(sentence_identification, language[language_prediction.lower()], language[language_selected])
